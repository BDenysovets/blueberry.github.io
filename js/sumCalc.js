const sumCalc = () => {
    ...
    if (+userValue > 5 && +userValue <= 100) {
        totalSum = Math.round((+userValue * 110) / 100 * 95);
        totalDiscount = (+userValue * 110) / 100 * 5;
        discount.innerHTML = `5 % - ${totalDiscount} грн`;
        sum.innerHTML = `${totalSum} грн`;
    } else if (+userValue > 100) {
        totalSum = Math.round((+userValue * 110) / 100 * 90);
        totalDiscount = (+userValue * 110) / 100 * 10;
        discount.innerHTML = `10 % - ${totalDiscount} грн`;
        sum.innerHTML = `${totalSum} грн`;
    } else if (userValue == "" || +userValue === 0 || +userValue <= 5) {
        totalSum = Math.round((+userValue * 120));
